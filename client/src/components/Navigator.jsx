...
export default function Navigator() {
  ...
  return (
    <>
      <aside
        className={`sm:hidden fixed z-10 ${
          isOpen ? "top-0" : "top-[-100%]"
        } h-auto w-full bg-gray-800 duration-300`}
      >
        <FaTimes
          className={`${
      ...
      <nav className="flex justify-between lg:text-base items-center pt-5 max-w-screen-2xl container mx-auto">
        ...
        <ul className="relative flex justify-between sm:justify-center items-center p-3 w-full sm:w-auto">
          <Link
            ...
            className={`hidden sm:flex justify-center items-center ${
              location.pathname === "/product/cart"
                ? " bg-gray-100 p-3 duration-500 rounded-full"
                : "p-3"
            }`}
          >
            <FiShoppingCart
              className={`text-3xl text-gray-500 hover:text-gray-800 duration-500 cursor-pointer`}
            />
            {cart.length !== 0 && (
              <span className="absolute border bg-[#E04132] top-[5px] left-[43px] text-white rounded-full w-5 h-5 text-center text-[12px] flex justify-center items-center">
                {cart.length}
              </span>
            )}
          </Link>
          ...
          <li className="text-gray-400 flex justify-center items-center gap-1 cursor-pointer">
            ...
            <Link
              to="/product/cart"
              className={`sm:hidden block text-3xl text-gray-500 ${
                location.pathname === "/product/cart"
                  ? " bg-gray-100 p-3 duration-500 rounded-full"
                  : "p-3"
              }`}
            >
              <div className="relative">
                <FiShoppingCart />
                {cart.length !== 0 && (
                  <span className="absolute border bg-[#E04132] top-[-19px] left-[16px] text-white rounded-full w-5 h-5 text-center text-[12px] flex justify-center items-center">
                    {cart.length}
                  </span>
                )}
              </div>
            ...
            {isAuthenticated ? (
              <p
                className="hidden sm:block cursor-pointer text-orange-500 font-semibold hover:text-[#E04132] duration-500"
                onClick={() => handleLogout()}
              >
                Log out
              </p>
