        ...
        >
          ...
          {!isDragActive && (
            ...
            >
              ...
              <p className="font-extralight text-sm text-gray-400 text-center">
          ...
          {isDragReject && (
            <p className="font-bold text-3xl text-red-800 text-center">
              {" "}
              😯 Sorry! Product images only accept JPG or PNG formats.
            </p>
          )}
        </div>
      )}
        ...
        {avatars[0]?.id
          ? avatars.map((file) => {
              return (
                <div
                  key={file.id}
                  className="relative flex flex-col justify-center items-center w-32"
                >
                  <img
                    src={file.preview}
                    alt={file.name}
                  ...
                  />
                  <button
                    onClick={(e) => handleRemoveWhenUpload(e, file.id)}
                    className="absolute top-[-10px] right-[-10px] rounded-full bg-[#E04132] text-white w-8 h-8"
                  >
                    x
                  </button>
                </div>
          ...
          : avatars.map((file) => {
              return (
                ...
                >
                  ...
                  />
                  <button
                    onClick={(e) => handleRemoveWhenEdit(e, file.publicId)}
                    className="absolute top-[-10px] right-[-10px] rounded-full bg-[#E04132] text-white w-8 h-8"
                  >
                    x
                  </button>
