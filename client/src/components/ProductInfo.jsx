...
const ProductInfo = ({ productDetail, handlePreview, preview, role }) => {
  ...
  const handleAddToCart = () => {
    const newCart = [...cart];
    const productExist = newCart.some(
      (item) => item._id === productDetail[0]._id
    );
    if (productExist) {
      ...
      setCart([
        ...cart,
        { ...productDetail[0], quantity: 1, amount: productDetail[0].price },
      ]);
      toast({
    ...
    <>
      <div className="xl:w-[50%] flex flex-col-reverse xl:flex-col justify-center gap-5 sm:gap-12">
        <section className="w-full xl:h-[340px] px-5 text-center xl:text-start flex flex-col justify-evenly gap-3">
          <h1 className="w-full pt-5 xl:pt-0 font-bold text-3xl xl:text-5xl text-center">
            {productDetail[0]?.name}
          </h1>
          <p className="text-gray-400 text-center text-xl">
            {productDetail[0]?.code}
          </p>
          <p className="text-[#E04132] font-bold text-4xl">
          ...
          <div className="w-full pb-10 xl:pb-0 mt-2 flex justify-center items-center gap-5 py-5">
            <button
              onClick={
                isAuthenticated ? () => handleAddToCart() : () => handleAlert()
              }
                ...
                <img
                  ...
                  className={`${
                    item.publicId === preview[0].publicId
                      ? "opacity-100"
                      : "opacity-20"
                  } object-cover w-14 sm:w-20 h-18 sm:h-24  rounded-xl`}
